...
TAU = 0.005
BATCH_SIZE = 256
POST_PROCESS_IMAGE_SIZE = (96, 96, 1)
DELAY_TRAINING = 500
NUM_FRAMES = 4
...
def image_preprocess(image, new_size=(96, 96)):
...
class Memory:
    def __init__(self, max_memory):
        ...
        self._actions = np.zeros((max_memory,num_actions), dtype=np.int32)
        self._rewards = np.zeros((max_memory,num_actions), dtype=np.float32)
...
def policy(state, t):
    p = np.array([q[(state,x)]/t for x in range(num_actions)])
...
def choose_action(state, primary_network, eps, step):
    if step < DELAY_TRAINING:
        return np.random.choice(num_actions, num_actions)
    else:
    ...
    while True:
        ...
        if i % MODEL_SAVE_FREQ == 0: # and i != 0:
            ...
            try:
                ...
                print(param)
                input_shape= (num_actions,96,96,4)
